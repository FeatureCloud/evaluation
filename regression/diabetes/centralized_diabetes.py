...
def perform_centralized_analysis_diabetes():
    ...
    def plot_boxplots(df, title):
        fig = go.Figure()
        fig.add_trace(go.Box(y=df["mean_absolut_error"].map(np.log2), quartilemethod="linear", name="Mean Absolut Error"))
    ...
    for model in models:
        ...
        for i in range(1, 11):
            ...
            splits.append(pd.read_csv(f'regression/diabetes/data/10-cv-splits/split_{i}.csv'))
        ...
        plt = plot_boxplots(score_df, title=f'Diabetes: {model}')
        score_df.to_csv(f'regression/diabetes/centralized_results/{model}_sklearn.csv', index=False)
        plt.write_image(f'regression/diabetes/centralized_results/{model}_sklearn.pdf')
