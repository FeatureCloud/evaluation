...
app_images = ['featurecloud.ai/flimma']
...
generic_dir = f'tests/fc_paper/flimma/generic/'

# this example runs the analysis on the data in data_dir/dataset/n_clients

for dataset in ['flimma']:
    for i in [5]:
        print(f'Run dataset {dataset} {i}')
        input_path = f'{data_dir}/tests/fc_paper/{dataset}/{i}_clients_2/'
        run_workflow(app_images, data_dir, generic_dir, input_path, 'http://localhost:8002')
        sleep(1)
        print(f'Finished run dataset {dataset} {i}')
sleep(5)
